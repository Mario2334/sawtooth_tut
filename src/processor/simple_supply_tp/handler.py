...
from src.protobuf.simple_supply_protobuf import payload_pb2

from src.processor.simple_supply_tp.state import SimpleSupplyState
...
def _transfer_record(state, public_key, payload):
    ...
    if not _validate_record_owner(signer_public_key=public_key,
                                  record=record):
        raise InvalidTransaction('Transaction signer is not the owner of the record')
